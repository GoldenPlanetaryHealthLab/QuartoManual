...
::: {.manual-prereq title="Confirm Package Declarations"}

Confirm that the platform and language package declarations exist before
writing the runtime activation contract.
...
:::: {.manual-procedure title="Write The Activation Contract"}
...
::: {.manual-check title="Verify The Activation Script"}

Confirm that the activation script exists, is executable, and contains the
canonical entrypoint.

```{python}
#| eval: false
assert project_contract.activate_script.exists()
assert project_contract.activate_script.stat().st_mode & 0o111
assert "source /work/env/activate.sh" in project_contract.activate_script.read_text()
```
