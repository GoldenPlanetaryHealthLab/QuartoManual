---
title: "Install Spack Layer"
format: html
execute:
  eval: false
---

::: {.manual-prereq title="Confirm Activation And Spack Declarations"}

Confirm that activation and dependency declaration artifacts exist before
submitting the Spack installation.

```{bash}
#| eval: false
test -s env/activate.sh
test -s spack.yaml
```

:::

:::: {.manual-procedure title="Install The Spack Layer"}

This notebook installs the project Spack layer. Spack works by creating
the declared project dependencies. Installation may be submitted to Slurm; use
the configured partition, CPU, memory, and time values for the target cluster.
...
::: {.manual-check title="Verify The Spack Layer"}

Confirm that the installation job completed successfully and that the project
Spack environment can be queried from the activated runtime.
