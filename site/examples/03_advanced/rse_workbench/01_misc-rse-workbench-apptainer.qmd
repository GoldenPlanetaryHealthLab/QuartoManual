---
title: "Helper Functions: Singularity/Apptainer"
format: html
execute:
  eval: false
---
...
Run this page from the workbench project root and confirm that the helper
dependencies used by the page are available.
...
:::: {.manual-procedure title="Define Apptainer Helpers"}
...
::: {.manual-check title="Check Helper Definitions"}

Confirm that the page leaves the two helper definitions available for later
container and scheduler procedures.

```{python}
#| eval: false
assert callable(spython_exec)
assert callable(show_spython)
```
