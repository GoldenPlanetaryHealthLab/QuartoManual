...
::: {.manual-prereq title="Confirm The Dependency Layer"}

Confirm that the project contract and platform dependency declaration are
available before writing language package declarations.
...
:::: {.manual-procedure title="Declare Language Packages"}
...
::: {.manual-check title="Verify Language Declarations"}

Confirm that both language package declaration files exist and are non-empty.

```{python}
#| eval: false
assert project_contract.rproject_toml.exists()
assert project_contract.pyproject_toml.exists()
assert project_contract.rproject_toml.stat().st_size > 0
assert project_contract.pyproject_toml.stat().st_size > 0
```
