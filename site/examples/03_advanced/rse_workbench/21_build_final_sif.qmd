...
::: {.manual-prereq title="Confirm All Runtime Validations"}

Confirm that platform, activation, R, language packages, and Quarto validation
have completed before building the final read-only image.
...
:::: {.manual-procedure title="Build And Validate The Final Image"}
...
::: {.manual-check title="Verify The Final Image"}

Confirm that the final SIF exists and that the runtime, Spack, R, and Quarto
checks report the expected installations.

```{python}
#| eval: false
assert project_contract.build_image.exists()
assert project_contract.build_image.stat().st_size > 0
```
