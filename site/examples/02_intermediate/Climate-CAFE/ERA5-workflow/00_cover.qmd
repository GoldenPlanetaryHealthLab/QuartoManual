...
user to digest, and it is not interactive, can only be read as greyed-out
text in your editor, and is not executable. A Quarto Manual allows us to couple 
the prose and code into a more interactive, executable, and digestible format
by decorate our prose with headers, callouts, tables, and other formatting to 
make it more readable and engaging, which lowers the barrier to entry for users
for very little additional effort.

To demonstrate this, I prompted ChatGPT to convert the Climate-CAFE 
scripts into a Quarto Manual based on the extension's thesis you read in the introductory sections. 

How does it compare to reading each script here?:

- [01_Query_ERA5_CDSAPI_Kenya_v1.py](https://github.com/Climate-CAFE/era5-daily-heat-aggregation-python/blob/main/code/01_Query_ERA5_CDSAPI_Kenya_v1.py)
- [02_Aggregate_ERA5_Kenya_SetFishnet_v1.py](https://github.com/Climate-CAFE/era5-daily-heat-aggregation-python/blob/main/code/02_Aggregate_ERA5_Kenya_SetFishnet_v1.py)
- [03_Aggregate_ERA5_Kenya_UseFishnet_v1.py](https://github.com/Climate-CAFE/era5-daily-heat-aggregation-python/blob/main/code/03_Aggregate_ERA5_Kenya_UseFishnet_v1.py)
...
For optimal learning, the section below should instead be coupled code and prose blocks in each individual Manual Page...
...
The section below could be implemented as the first page of a Quarto Manual, with each dependency being a prerequisite check for the operator to confirm before proceeding to the next step.
A pytest suite would be very easy to implement here, and on page two, to check for.
...
With Quarto, we can take advantage of the rich publishing ecosystem to include references directly from `bibtex` files, rather than having to manually format them in the markdown as below.
